...
//     return await supabase.from('pages').update({ content }).eq('id', id).select()
// }

// export async function addPage(user_id: string, parent_id: null | string = null) {
//     return await supabase.from('pages').insert({ user_id, parent_id }, { count: 'exact' }).select()
// }
